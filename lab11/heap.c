    ...
    if (compare(heap->data[idx], heap->data[PARENT(idx)]) > 0)
        {
            ...
            shiftUp(heap,PARENT(idx),compare);
        }
...
heap_err_t shiftDown(heapArray_t *heap, size_t idx, int (*compare)(const void *, const void *)) {
    ...
    int i,j;
    if (LEFT_CHILD(idx) == heap->size-1)
        {
            i = LEFT_CHILD(idx);
            if (compare(heap->data[idx], heap->data[LEFT_CHILD(idx)]) < 0)
                swap(heap,idx,i);
                return STATUS_OK;
        }
    ...
    if (compare(heap->data[idx], heap->data[i]) < 0)
    {
        swap(heap,idx,i);
        shiftDown(heap,i,compare);
        return STATUS_OK;
    }
    
    if (compare(heap->data[idx], heap->data[j]) < 0)
    {
        swap(heap,idx,j);
        shiftDown(heap,j,compare);
        return STATUS_OK;
    }
...
void insertHeapNode(heapArray_t **heap, void *value, size_t dataSize, int (*compare)(const void *a, const void *b)) {
    ...
    memcpy((*heap)->data[(*heap)->size-1] , value, dataSize);

    shiftUp(*heap,(*heap)->size-1,compare);
